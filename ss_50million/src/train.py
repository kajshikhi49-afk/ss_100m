...
def train_qlora(config=None, corpus_path="data/corpus.txt", base_checkpoint=None):
    ...
    print("=" * 65)
    print("🚀 50M BENGALI GPT — QLoRA ফাইন-টিউনিং শুরু হচ্ছে (২০৪৮ কনটেক্সট লেন্থ)")
    ...
    ds = BengaliDataset(corpus_path, block_size=config.block_size)
    ...
    print(f"🎉 ৫০M মডেলের QLoRA ফাইন-টিউনিং সম্পন্ন! সেরা ভ্যালিডেশন লস: {best_val:.4f}")
    return model
...
def train_pretrain(config=None, corpus_path="data/corpus.txt"):
    ...
    print("=" * 65)
    print("🚀 50M BENGALI GPT — FULL PRETRAINING (২০৪৮ কনটেক্সট লেন্থ)")
    print("=" * 65)
    ...
    for step in range(start_step + 1, config.max_iters + 1):
        ...
        # প্রতি ২৫ স্টেপে লাইভ প্রগ্রেস প্রদর্শন
        if step % 25 == 0 and step % config.eval_interval != 0:
            elapsed = time.time() - t0
            steps_done = step - start_step
            speed = elapsed / max(1, steps_done)
            eta_mins = (config.max_iters - step) * speed / 60
            print(f"  ⚡ Step {step:4d}/{config.max_iters} | Loss: {accum_loss:.4f} | LR: {lr:.2e} | গতি: {speed:.2f}s/step | বাকি: ~{eta_mins:.1f} মিনিট")

        if step % config.eval_interval == 0 or step == config.max_iters:
            losses = _eval_loss(model, ds, config)
            elapsed = time.time() - t0
            print(f"Step {step:5d}/{config.max_iters} | Train: {losses['train']:.4f} | Val: {losses['val']:.4f} | LR: {lr:.2e} | সময়: {elapsed/60:.1f} মিনিট")
            is_best = losses['val'] < best_val
    ...
    print(f"🎉 ৫০M মডেলের ফুল প্রি-ট্রেইনিং সম্পন্ন! সেরা লস: {best_val:.4f}")
    return model
